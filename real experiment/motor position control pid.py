...
def main_new():
    ...
    df = pd.read_csv("phi_opposite.txt")
    angle_sequence = df["opposite_phi"].tolist()
    time_sequence = df["timestamp"].tolist()
    hold_time = 0.05  
    # speed_val = 400  # movement speed (0.01 deg/s)
    Kp = 5/1
    Kd = 2/1
    Ki = 0.5/1
    # dt = 0.1
    ...
    while i < len(angle_sequence):
        ...
        if i == 0:
            ...
            dt = time_sequence[i] - time_sequence[i-1]
    ...
    df.to_csv("angle_feedback_comparison_pid.csv", index=False)
